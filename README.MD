...
subscription-manager release --set=8.1 #锁定特定版本
...
```
sha1sum scrt-sfx-8.7.1-2171.rhel7-64.x86_64.rpm #确认是faea170e1c23aa2b44f025fdc733a9c98d96b123
yum install scrt-sfx-8.7.1-2171.rhel7-64.x86_64.rpm
...
参考[Install Workstation Pro on a Linux Host](https://docs.vmware.com/en/VMware-Workstation-Pro/15.0/com.vmware.ws.using.doc/GUID-1F5B1F14-A586-4A56-83FA-2E7D8333D5CA.html)
```
...
sha1sum VMware-Workstation-Full-15.5.2-15785246.x86_64.bundle #确认是425daf98f58495e355538ef9685dd2e176d54782
sh VMware-Workstation-Full-15.5.2-15785246.x86_64.bundle --console --ignore-errors --set-setting vmware-workstation serialNumber GU7HH-24W15-M88AP-MYMXC-ZZATA
```
...
```
md5sum anyconnect-linux64-4.8.03036-predeploy-k9.tar.gz #确认是6ef1aa787e19311b9c499c7a88e2c493
tar zxvf anyconnect-linux64-4.8.03036-predeploy-k9.tar.gz
cd anyconnect-linux64-4.8.03036/vpn/
./vpn_install.sh
```
...
sha1sum wps-office-11.1.0.8865-1.x86_64.rpm #a5f0434683f6d82c5a6d88b5f7295d2a22f88142
yum install wps-office-11.1.0.8865-1.x86_64.rpm
```
...
mount -o loop rhel-8.1-x86_64-dvd.iso /mnt/rhel8iso/
mount -o loop supp-supplementary-8.1-rhel-8-x86_64-dvd.iso /mnt/rhel8suppiso/
...
```
nano /etc/fstab
	rhel-8.1-x86_64-dvd.iso	/mnt/rhel8iso	udf,iso9660	user,auto,loop	0 0
	supp-supplementary-8.1-rhel-8-x86_64-dvd.iso	/mnt/rhel8suppiso	udf,iso9660	user,auto,loop	0 0
```
